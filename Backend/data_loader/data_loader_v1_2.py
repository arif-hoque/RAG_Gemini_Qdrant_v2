...
def scrape_websites(websites, output_file):
    # Set up Selenium WebDriver
    options = Options()
    options.headless = False  # Run Chrome in headless mode
    driver = webdriver.Chrome(options=options)
    ...
    for website in websites:
        ...
        for url in all_urls:
            print("Scraping data from:", url)
            content = scrape_data(driver, url)
            if content:
                all_content += f"URL: {url}\n\n"
                all_content += content + "\n\n"

    # Write all accumulated content to a single text file
...
def scrape_data(driver, url):
    ...
    try:
        ...
        # Replace unwanted strings with an empty string
        unwanted_strings = [
            "Gigalogy Tutorial", "Overview", "Account and Project creation", "Credentials", 
            "Personalizer", "Project setup", "Environment setup", 
            "Integration of Catalogue information and user behavior data", "Training your data", 
            "Personalized search", "Personalized Image Search", "Personalized Feed", 
            "Recommend trending items", "Recommend similar items", "Recommend items purchase together", 
            "Dynamic Pricing", "Questionnaire", "MyGPT", "API Reference", "Release notes", 
            "Glossary", "HOME PRODUCT", "COMPANY", "NEWS", "CONTACT US", "G-Core", "SmartAds", 
            "Developers", "Corporate Profile", "Our Mission", "Our Team", "Careers", 
            "PRESS & CONTACT", "Request a Demo", "Press Room", "Events", "Experience Box", 
            "DEVELOPERS", "Partner Program", "Recommender - Sandbox", "Recommender - Documentation", 
            "Recommender - Tutorial", "© 2024 Gigalogy Inc. Privacy Policy Terms and Conditions Cookie Policy Commercial Disclosure"
        ]
...
if __name__ == "__main__":
    websites = ["https://gigalogy.com", "https://tutorial.gigalogy.com"]  # Inputted websites
    output_file = "all_scraped_data.txt"  # File to save the scraped data
    scrape_websites(websites, output_file)
